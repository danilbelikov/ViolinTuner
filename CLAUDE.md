...
Порядок шагов — spec §8. Сделаны шаги 1–2: тема и токены (`core/ui/theme`), каркас навигации (`navigation/`, заглушки в `feature/history`, `feature/settings`), домен (`core/domain`) и `PitchSource` / `FakePitchSource` (`core/audio`). `feature/live` — пустой экран-заполнитель без `LiveContract` / `LiveViewModel`; `MicPitchSource`, детектора и Hilt-биндинга `PitchSource` ещё нет. Обновляй этот абзац при закрытии шага.
