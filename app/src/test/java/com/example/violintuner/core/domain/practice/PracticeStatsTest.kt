...
class PracticeStatsTest {
    ...
    fun `fill levels switch on the spec thresholds`() {
        ...
        for ((minutes, level) in expected) {
            assertEquals("$minutes min", level, PracticeStats.fillLevel(minutes * MS_PER_MINUTE, config))
        }
        assertEquals("under a minute still counts", 1, PracticeStats.fillLevel(30_000, config))
        assertEquals("19:59 is level 1", 1, PracticeStats.fillLevel(20 * MS_PER_MINUTE - 1, config))
    }
