...
class EqualizerTest {
    ...
    private fun measuredDb(eq: com.example.violintuner.core.domain.sound.EqSettings, hz: Double): Double {
    ...
    fun `a switched-off equalizer and bands at zero draw a flat line`() {
        ...
        assertTrue("gains of a switched-off equalizer are not drawn", EqCurve.responseDb(off.copy(low = Shelf(200.0, 9.0)), RATE, frequencies, config).all { it == 0.0 })
    }
    ...
    fun `a narrower bell reaches less far`() {
        ...
        assertTrue("$wide vs $narrow", wide > narrow + 3)
    }
    ...
    fun `shelves move everything beyond their corner, the low cut takes the rumble away`() {
        ...
        assertTrue("the open G string is left alone: ${cut[2]}", cut[2] > -0.3)
    }
    ...
    fun `the curve on the screen is what the sound goes through`() {
        ...
        for (hz in doubleArrayOf(60.0, 250.0, 900.0, 3_000.0, 12_000.0)) {
            ...
            assertEquals("at $hz Hz", drawn, measuredDb(eq, hz), 0.1)
        }
    }
    ...
    fun `a band dragged while the sound plays does not click`() {
        ...
        val honestStep = 0.2 * 3.98 * 2 * Math.PI * 800 / RATE
