...
import com.violinjourney.app.shared.resources.splash_away
import com.violinjourney.app.shared.resources.splash_home
import com.violinjourney.app.shared.resources.splash_skip
import com.violinjourney.app.shared.resources.splash_stop
 ...
 */
@Composable
fun SplashRoute(kind: SplashKind, onDone: () -> Unit, modifier: Modifier = Modifier, viewModel: HomeViewModel) {
    ...
    CompositionLocalProvider(LocalReduceMotion provides reduce) {
        ...
        Box(modifier.fillMaxSize().background(Color.Black).clickable(interactionSource = remember { MutableInteractionSource() }, indication = null, onClickLabel = stringResource(Res.string.splash_skip)) { finish() }) {
            ScenePicture(rememberScene(kind.scene, mode, folder = "home"), description = stringResource(if (kind == SplashKind.AWAY) Res.string.splash_away else Res.string.splash_home), modifier = Modifier.fillMaxSize(), seconds = rememberSceneSeconds(), whole = true, centred = true)
            Column(
                ...
                Text(stringResource(if (kind == SplashKind.AWAY) Res.string.splash_away else Res.string.splash_home).uppercase(), color = colors.primary, style = MaterialTheme.typography.labelMedium.copy(letterSpacing = 1.5.sp, fontWeight = FontWeight.Bold))
                if (kind == SplashKind.HOME) {
                    ...
                    Text(if (at == 0) cityOf(0) else stringResource(Res.string.splash_stop, cityOf(at), at, JourneyRoute.stops.lastIndex), color = Color.White, style = MaterialTheme.typography.titleLarge, textAlign = TextAlign.Center)
