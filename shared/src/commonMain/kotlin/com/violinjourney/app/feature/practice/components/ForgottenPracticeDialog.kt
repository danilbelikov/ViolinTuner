...
fun ForgottenPracticeDialog(prompt: PracticePrompt.Forgotten, onIntent: (PracticePromptIntent) -> Unit, zone: TimeZone) {
    ...
    Dialog(
        ...
        Column(
            ...
            ClockIcon()
            Text(
                text = stringResource(Res.string.practice_forgotten_title),
                color = colors.onSurface,
                style = MaterialTheme.typography.headlineSmall.copy(fontSize = 24.sp, fontWeight = FontWeight.Bold),
            )
            val elapsedText = stringResource(Res.string.practice_forgotten_text, Formats.minutesInWords(prompt.elapsedMs))
            val lastSound = prompt.lastSoundEpochMs?.let { Formats.timeOfDay(it, zone) }
            Text(
                text = if (lastSound != null) {
                    elapsedText + " " + stringResource(Res.string.practice_forgotten_sound, lastSound)
                } else {
                    elapsedText
                },
            ...
            Column(modifier = Modifier.padding(top = 4.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                if (lastSound != null) {
                    MainButton(stringResource(Res.string.practice_end_at, lastSound)) { onIntent(PracticePromptIntent.EndAtLastSound) }
                    SecondaryButton(stringResource(Res.string.practice_end_now)) { onIntent(PracticePromptIntent.EndNow) }
                } else {
                    MainButton(stringResource(Res.string.practice_end_now)) { onIntent(PracticePromptIntent.EndNow) }
                    SecondaryButton(stringResource(Res.string.practice_edit_time)) { onIntent(PracticePromptIntent.EditTime) }
                }
                SecondaryButton(stringResource(Res.string.practice_continue)) { onIntent(PracticePromptIntent.Continue) }
            }
        }
    }
}
