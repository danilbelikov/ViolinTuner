...
import com.violinjourney.app.core.domain.home.HomeCatalogData
...
object ScenePalette {
    ...
    fun parse(value: String): Long? = when {
        ...
        value.startsWith("rgba") -> {
            ...
            if (n.size < 4) null else (floor(n[3] * 255 + 0.5).toLong() shl ALPHA_SHIFT) or (n[0].toLong() shl 16) or (n[1].toLong() shl 8) or n[2].toLong()
        }
        else -> null
    }
    ...
    fun mix(a: Long, b: Long, t: Float): Long {
        fun channel(shift: Int): Long {
            ...
            return floor(from + (to - from) * t + 0.5).toLong().coerceIn(0, 255) shl shift
        }
