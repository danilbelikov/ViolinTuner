...
import com.violinjourney.app.shared.resources.session_note_mean
...
import com.violinjourney.app.shared.resources.session_note_position
...
import com.violinjourney.app.shared.resources.session_note_target
...
@Composable
fun NoteSheetContent(segment: RollSegment, modifier: Modifier = Modifier, place: NotePlace? = null) {
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.Bottom) {
            Column(modifier = Modifier.weight(1f)) {
                Text(stringResource(Res.string.session_note_target), color = colors.onSurfaceVariant, style = MaterialTheme.typography.bodySmall.copy(fontSize = 13.sp))
                Row {
                    Text(
            ...
            Column(horizontalAlignment = Alignment.End) {
                Text(stringResource(Res.string.session_note_mean), color = colors.onSurfaceVariant, style = MaterialTheme.typography.bodySmall.copy(fontSize = 13.sp))
                Text(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            Tile(
                label = stringResource(Res.string.session_note_min_max),
                value = stringResource(
                    Res.string.session_note_min_max_value,
                    Formats.signedCents(segment.minCents), Formats.signedCents(segment.maxCents),
                ),
                modifier = Modifier.weight(1f),
            )
            Tile(
                label = stringResource(Res.string.session_note_duration),
                value = stringResource(
                    Res.string.session_note_duration_value,
                    Formats.oneDecimal((segment.endMs - segment.startMs) / MS_PER_SECOND),
                ),
                modifier = Modifier.weight(1f),
            )
            Tile(
                label = stringResource(Res.string.session_note_range),
                value = stringResource(
                    Res.string.session_note_range_value,
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(12.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Column {
                Text(
                    text = stringResource(
                        Res.string.session_note_position,
                ...
                Text(
                    text = stringResource(
                        when {
                            segment.zone == Zone.IN_TUNE && !segment.steady -> Res.string.session_tip_wandering
                            segment.zone == Zone.IN_TUNE -> Res.string.session_tip_stable
                            segment.meanCents < 0 -> Res.string.session_tip_flat
                            else -> Res.string.session_tip_sharp
                        },
                    ),
                    color = colors.onSurfaceVariant,
        ...
        if (place != null) {
            val label = stringResource(if (place.video) Res.string.video_watch_place else Res.string.video_listen_place)
            if (place.video) {
