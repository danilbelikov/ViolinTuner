...
class SoundChainTest {
    ...
    fun `every preset keeps a loud recording under the ceiling and stays sane`() {
        ...
        for (preset in BuiltInPreset.entries) {
            ...
            chain.process(output)
            assertTrue(output.all { it.isFinite() }, "$preset")
            assertTrue(FxSignals.peak(output) <= ceiling + 1e-6, "$preset peaks at ${FxSignals.peak(output)}")
        }
    }
    ...
    fun `the meters tell the level — the squeeze and the limiter — and start anew once taken`() {
        ...
        assertTrue(loud.reductionDb > 3.0, "squeezes by ${loud.reductionDb}")
        assertTrue(loud.limiting, "the limiter works on +12 dB")
        ...
        assertTrue(calm.outputPeakDb < -40, "a quiet note reads ${calm.outputPeakDb}")
