...
class SoundParamsTest {
    ...
    fun `every number can be written and read back — within its range`() {
        SoundParam.entries.forEach { param ->
            ...
            assertEquals(middle, SoundParams.get(param, SoundParams.set(param, middle, off, config))!!, 1e-9, param.name)
            assertEquals(range.max, SoundParams.get(param, SoundParams.set(param, 1e9, off, config))!!, 1e-9, param.name)
            assertEquals(range.min, SoundParams.get(param, SoundParams.set(param, -1e9, off, config))!!, 1e-9, param.name)
        }
    }
    ...
    fun `nothing but the number asked for changes`() {
        ...
        assertEquals(false, changed.eq.enabled, "the switch of the block is not the slider's business")
    }
    ...
    fun `the knob leads its five — and a hand on any of them lets it go`() {
        ...
        assertEquals(byKnob.compressor.thresholdDb, byHand.compressor.thresholdDb, 0.0, "the other four stay where the knob left them")
    }
    ...
    fun `the tail is as long as its space allows`() {
        ...
        assertEquals(5.0, SoundParams.range(SoundParam.REVERB_PRE_DELAY, room, config).default, 0.0, "a double tap returns to the pre-delay of the space")
    }

    @Test
    fun `frequencies lie on a logarithmic track — the rest on a straight one`() {
        val whole = ParamRange(20.0, 20_000.0, 1_000.0)
        assertEquals(632.5, SoundParams.valueAt(SoundParam.BODY_HZ, 0.5f, whole), 0.1, "the middle of 20 Hz – 20 kHz")
        assertEquals(0.5f, SoundParams.fractionOf(SoundParam.BODY_HZ, 632.46, whole), 1e-3f)
        assertEquals(109.5, SoundParams.valueAt(SoundParam.LOW_CUT_HZ, 0.5f, config.lowCutHz), 0.1, "the middle of the low cut")
    ...
    fun `a press of plus or minus is one step of the unit`() {
        ...
        assertEquals(2.5, SoundParams.stepped(SoundParam.OUTPUT_GAIN, 2.2, up, config.outputGainDb), 1e-9, "an odd value lands on the grid")
