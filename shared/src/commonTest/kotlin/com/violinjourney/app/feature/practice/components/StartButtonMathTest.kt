    ...
    fun `the centre of every light stays on the button`() {
        for (index in lights) {
            ...
            while (t < TEN_MINUTES) {
                ...
                assertTrue(centre.x in 0f..1f, "light $index at $t s: x ${centre.x}")
                assertTrue(centre.y in 0f..1f, "light $index at $t s: y ${centre.y}")
                t += STEP
            }
        }
    }
    ...
    fun `no light crosses a button faster than the limit — however wide the button`() {
        for (widthDp in listOf(LANDSCAPE_DP, PORTRAIT_DP, TABLET_DP)) {
            ...
            for (index in lights) {
                ...
                assertTrue(fastest <= StartButtonMath.MAX_SPEED_DP_S, "light $index on $widthDp dp: $fastest dp/s")
            }
        }
    }
